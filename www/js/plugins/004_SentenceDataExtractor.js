...
(function () {
  //
  var parameters = PluginManager.parameters('004_SentenceDataExtractor');
  ...
  for (num = 1; num < jsonmapd.length; num++) {
    var filePath = projectFilePath + '/data/Map' + ("00" + num).slice(-3) + '.json';
    ...
    if (isExist40x) {
      fs.writeFileSync(projectFilePath + '/textOutput/' + jsonmapd[num].name + '.txt', dataStr);
      //fs.writeFileSync(projectFilePath+'/'+jsonmapd[num]+'.txt', dataStr);
    }
  }
